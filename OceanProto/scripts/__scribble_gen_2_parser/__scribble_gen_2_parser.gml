...
// Fix for OceanProto project! See https://github.com/JujuAdams/Scribble/issues/347
// There's a bug that the getter here won't work on HTML5 builds, so we need a different implementation (the second one here) for those builds...
// But there isn't a way to set a macro based on build platform. There is a way to do that based on build configuration though,
// so let's do it for the "Release" and "Demo" configs, which happen to be the ones we always use for HTML5 builds.
#macro __S_GLYPH_GET         var _data_index = _font_glyphs_map[? _glyph_write];
#macro Release:__S_GLYPH_GET var _data_index = _font_glyphs_map[? string(_glyph_write)];
#macro Demo:__S_GLYPH_GET    var _data_index = _font_glyphs_map[? string(_glyph_write)];
